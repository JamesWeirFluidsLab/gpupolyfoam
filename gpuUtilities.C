...
// Initialise customNonbonded force for OpenMM	
    CustomNonbondedForce* cnonBonded;
    //another force nonbonded for ewald coulumb interactions
    ...
    if(scallingfunction == "shiftedForce"){
        ...
        // ================================================
        cnonBonded = new CustomNonbondedForce(formulastr);
        cnonBonded->addGlobalParameter("rCut",solver->rCutInNM);
        //initialise nonbonded class
        nonbonded = new NonbondedForce();
        nonbonded->setCutoffDistance(solver->rCut);
 ...
 */
    extractOFParticles(solver,cnonBonded);
...
//                        params[species-1] = tempmolcharge;
			...
			for(int i=0;i<molsize-1;i++)
                for(int j=i+1;j<molsize;j++){
                    cnonBonded->addExclusion(midx[i],midx[j]);
...
//    if(coefftype == "lennardJones")
//        cnonBonded->addPerParticleParameter("q");
